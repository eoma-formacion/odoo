...
class EventMinimalController(WebsiteEventController):
    ...
    def _prepare_minimal_event_values(self, event, **kwargs):
        ...
        available_tickets = event.event_ticket_ids.filtered(
            lambda t: t.is_launched and t.sale_available and not t.is_expired
        )
    ...
    def event_registration_confirm(self, event, **post):
        ...
        if is_minimal:
            ...
            if hasattr(result, 'location') and 'registration/success' in result.location:
                # Check if payment is required
                registration_ids = request.session.get('registration_ids', [])
                if registration_ids:
                    # Get the first registration to check for payment
                    registration = request.env['event.registration'].sudo().browse(registration_ids[0])
                    if registration and registration.event_id.website_menu and hasattr(registration, 'payment_transaction_id'):
            ...
            for ticket_id, attendees in form_details.items():
                ...
                for attendee_data in attendees:
                    ...
                    if not attendee_data.get('name') or not attendee_data.get('email'):
                        return {'error': _('Name and email are required for all attendees')}
    ...
    @http.route(['/event/checkout/create_order'], type='json', auth='public', website=True)
    ...
    def _validate_checkout_data(self, event, attendees, billing=None):
        ...
        for ticket_id, count in ticket_counts.items():
            ...
            if ticket.seats_max and ticket.seats_max > 0 and ticket.seats_available < count:
                errors[f'ticket_{ticket_id}'] = _('Not enough tickets available. Only %d remaining.') % ticket.seats_available
    ...
    def _create_sales_order(self, event, partner, attendees):
        ...
        for attendee in attendees:
            ticket = request.env['event.event.ticket'].sudo().browse(int(attendee['ticket_id']))
        ...
        try:
            ...
            for ticket_id, quantity in form_data.get('tickets', {}).items():
                if int(quantity) > 0:
                    ...
                    if ticket.seats_max and ticket.seats_max > 0 and ticket.seats_available < int(quantity):
                        errors[f'ticket_{ticket_id}'] = _('Not enough tickets available')
